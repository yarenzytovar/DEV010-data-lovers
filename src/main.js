...
import { filtroCategoriaSerie, funcionDeOrdenarDatos } from "./data.js";
...
ordenAlfabetoeAsc.addEventListener("change", (event) => {
  ...
  const dataOrdenada = funcionDeOrdenarDatos(orden,datosPersonajes.breaking_bad);
...
selectSerie.addEventListener("change", (event) => {
  ...
  //const serieSeleccionada = datosPersonajes.breaking_bad.filter((personaje) => {
  //return personaje.category.includes(serie);
  // const serieSeleccionada = datosPersonajes.breaking_bad.filter((elem) => elem.category.includes(serie));
...
//for (const personaje of datosPersonajes.breaking_bad) {
//const apariciones = personaje.appearance;
  
//let suma = 0;

//for (const numero of apariciones) {
//suma += numero;
//}

//const promedio = suma / apariciones.length;
//console.log(`El promedio de ${personaje.name} es ${promedio}`);
